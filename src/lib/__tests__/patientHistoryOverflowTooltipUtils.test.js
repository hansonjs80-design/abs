...
test('patient history cells support range selection, direct typing, and fill handles', async () => {
  ...
  assert.match(
    shockwaveCss,
    /\.patient-history-table tbody tr:hover > td[\s\S]*?font-weight:\s*800\s*!important;/,
  );
  assert.doesNotMatch(
    shockwaveCss,
    /\.patient-history-table tbody tr:hover > td[\s\S]*?background-color:\s*#94a3b8 !important;/,
  );
