...
test('context menu body-part checkbox rows use stronger hover backgrounds', async () => {
  ...
  assert.match(
    css,
    /\.context-menu-body-preset-list\s*\{[^}]*gap:\s*2\.1px;[^}]*width:\s*calc\(100% - 5px\);[^}]*margin-left:\s*5px;/s
  );
  assert.match(
    css,
    /\.context-menu-body-presets\s*\{[^}]*gap:\s*7\.2px;[^}]*margin-bottom:\s*8px;[^}]*padding:\s*0 12px 4px 12px !important;/s
  );
  assert.match(
    css,
    /\.context-menu-body-preset-title\s*\{[^}]*margin:\s*0 0 2\.5px;[^}]*font-size:\s*calc\(0\.96rem \+ 1px\);/s
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /\.context-menu-body-preset-item:hover\s*\{[^}]*background:\s*#e5e7eb;/s
  );
  assert.match(
    css,
  ...
  assert.match(
    css,
    /\.context-menu-body-preset-label input\[type="checkbox"\]\s*\{[^}]*width:\s*14px;[^}]*height:\s*14px;[^}]*min-width:\s*14px;[^}]*align-self:\s*center;[^}]*margin:\s*0 6px 0 1px;/s
  );
  assert.match(
    css,
