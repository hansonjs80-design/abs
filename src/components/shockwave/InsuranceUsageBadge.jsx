import { INSURANCE_USAGE_COLORS, INSURANCE_USAGE_LABELS } from '../../lib/insuranceUsageUtils';
...
export default function InsuranceUsageBadge({ usage, status = '조회 중', showLabel = false }) {
  ...
  return <span title={`병원 내 관리용 · ${period}${usage.selfPay ? ' · (본인) 처방은 횟수 증가 제외' : ''}`}
    style={{ color: INSURANCE_USAGE_COLORS[usage.category], fontWeight: 800, whiteSpace: 'nowrap' }}>
    {showLabel ? `${INSURANCE_USAGE_LABELS[usage.category]} ` : ''}{usage.count}회
  </span>;
}
