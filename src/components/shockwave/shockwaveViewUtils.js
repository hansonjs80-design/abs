...
const PATIENT_HISTORY_DATE_COLUMN_INDEX = 1;
const PATIENT_HISTORY_CHART_COLUMN_INDEX = 2;
const PATIENT_HISTORY_PRESCRIPTION_COLUMN_INDEX = 3;
const PATIENT_HISTORY_BODY_COLUMN_INDEX = 4;
const PATIENT_HISTORY_MEMO_COLUMN_INDEX = 5;
const PATIENT_HISTORY_DATE_COLUMN_SCALE = 1.1;
const PATIENT_HISTORY_CHART_COLUMN_SCALE = 1.1;
const PATIENT_HISTORY_BODY_COLUMN_SCALE = 1.1;
...
const PATIENT_HISTORY_COLUMN_WIDTH_SCALE = (
  PATIENT_HISTORY_BASE_COLUMN_WIDTHS.reduce((sum, width, index) => {
    if (index === PATIENT_HISTORY_DATE_COLUMN_INDEX) {
      return sum + width * PATIENT_HISTORY_DATE_COLUMN_SCALE;
    }
    if (index === PATIENT_HISTORY_CHART_COLUMN_INDEX) {
      return sum + width * PATIENT_HISTORY_CHART_COLUMN_SCALE;
    }
    if (index === PATIENT_HISTORY_BODY_COLUMN_INDEX) {
      return sum + width * PATIENT_HISTORY_BODY_COLUMN_SCALE;
    }
...
export function getPatientHistoryColumnWidths(
  ...
  const expandedWidths = PATIENT_HISTORY_BASE_COLUMN_WIDTHS.map((width, index) => (
    index === PATIENT_HISTORY_DATE_COLUMN_INDEX
      ? width * PATIENT_HISTORY_DATE_COLUMN_SCALE
      : index === PATIENT_HISTORY_CHART_COLUMN_INDEX
        ? width * PATIENT_HISTORY_CHART_COLUMN_SCALE
      : index === PATIENT_HISTORY_BODY_COLUMN_INDEX
        ? width * PATIENT_HISTORY_BODY_COLUMN_SCALE
