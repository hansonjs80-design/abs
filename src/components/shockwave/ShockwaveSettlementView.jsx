...
function getIncentiveRateSummary(summaries, percentage) {
  return (Array.isArray(summaries) ? summaries : []).find(
    (summary) => Number(summary?.percentage) === Number(percentage)
...
export default function ShockwaveSettlementView({
  ...
  const settlement = useMemo(() => {
    ...
    return {
      ...
      grandAmount,
      grandIncentive,
      grandIncentiveRateBreakdown,
      incentiveRatePercentages: grandIncentiveRateBreakdown.map((summary) => summary.percentage),
    };
  }, [
    displayTherapists,
                    ...
                    <table className="sw-vertical-therapist-table">
                      ...
                      <tbody>
                        ...
                        <tr className={`vertical-total-row therapist-tone-${therapistIndex % 3}${showIncentiveRateSubtotals ? ' settlement-rate-total-row' : ''}`}>
                          <th>{showIncentiveRateSubtotals ? '전체 합계' : '합계'}</th>
                ...
                <div className="sw-settlement-table-wrap">
                  <table className="sw-vertical-summary-table">
                    ...
                    <tbody>
                      ...
                      <tr className={`grand-total-row${showIncentiveRateSubtotals ? ' settlement-rate-total-row' : ''}`}>
                        <th>{showIncentiveRateSubtotals ? '전체 합계' : '총 합계'}</th>
